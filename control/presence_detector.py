...
Mục đích: Bộ não điều khiển chu trình nhận diện khuôn mặt.
[UPDATE]: Tiết kiệm điện năng - Tích hợp cơ chế tự động Bật/Tắt Camera.
Chỉ đánh thức Camera khi PIR phát hiện người, quét xong tự ngủ đông.
"""
from __future__ import annotations
...
class PresenceDetector:
    ...
    async def on_pir_triggered(self) -> None:
        if self._state != State.IDLE: return
        log_app("info", "[DETECTOR] PIR triggered → WATCH. Đang đánh thức Camera...")
        
        # ✅ TIẾT KIỆM PIN: Bật camera ngay khi PIR kích hoạt
        try:
            await self._camera.camera_on()
            await asyncio.sleep(0.5) # Đợi 0.5s cho cảm biến ảnh khởi động & điều chỉnh sáng (Warm-up)
        except Exception as e:
            log_app("error", f"[DETECTOR] Lỗi bật camera: {e}")
            return
    ...
    async def _main_loop(self) -> None:
        try:
            ...
            if not face_found:
                log_app("info", "[DETECTOR] Không thấy mặt ai → Về ngủ (IDLE)")
                await self._emit("face_scan_end", {"reason": "no_face"})
                await self.reset() # Hàm reset sẽ kiêm luôn việc tắt Camera
                return
    ...
    async def _scan_phase(self) -> None:
        ...
        for i in range(1, MAX_SCAN_RETRIES + 1):
            ...
            # ✅ LOGIC MƯỢT HƠN: Chỉ báo UART "Đang quét" khi đã lấy được ảnh thành công
            if self._uart_send:
                await self._uart_send(f"CMD_SCAN_FACE:{i}\n")
    ...
    async def reset(self) -> None:
        ...
        # ✅ TIẾT KIỆM PIN: Quét xong (dù thành công hay thất bại) đều tắt mắt thần đi cho máy nghỉ ngơi
        try:
            await self._camera.camera_off()
            log_app("info", "[DETECTOR] Đã đưa Camera vào trạng thái Ngủ đông (IDLE)")
        except Exception as e:
            log_app("error", f"[DETECTOR] Lỗi tắt camera: {e}")