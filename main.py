...
# # # CHANGES:
# # #   [FIX] PresenceDetector now receives broadcast_fn parameter
# # #         → broadcasts face_scan_start/end/recognized to Dashboard via backend WebSocket
# # #   [FIX] /sync-faces and /delete-face endpoints use X-Api-Key auth
# # #   [FIX] face_sync_pull_loop as background task (fallback when Pi was offline)
# # #   [FIX] BUG #4 & WARN #2: Thêm ký tự \n vào các lệnh UART (Warning Beep, Test Buzzer)
# # #   [KEEP] UART, PIN sync, alarm, door control UNCHANGED
# # # """
# # # from __future__ import annotations
...
# # #     """
# # #     Pi calls this to push real-time events to backend WebSocket,
# # #     which forwards to Dashboard. Used for face_scan_start/end/recognized.
# # #     """
...
# # #     async def on_panic_alarm():
# # #         log_app("warning", "Too many failed scans → alarm")
# # #         await alarm_ctrl.start()

# # #     # PresenceDetector: pass broadcast_fn so it can notify Dashboard
# # #     detector = PresenceDetector(
# # #         camera=shared_camera,
# # #         on_recognized=on_face_ok,
# # #         on_unknown=on_face_fail,
# # #         on_alarm=on_panic_alarm,
# # #         broadcast_fn=_broadcast_to_backend,   # NEW
# # #     )
...
# # #     async def handle_unlock(request):
# # #         # Sẽ hoạt động hoàn hảo sau khi chúng ta vá file door_controller.py
# # #         await door_ctrl.unlock()
# # #         return web.json_response({"success": True})
...
# # #         """POST /sync-faces — backend pushes encoding to Pi after Dashboard enroll."""
...
# # #         """POST /delete-face — backend notifies Pi when resident is deleted."""
# # #         api_key = request.headers.get("X-Api-Key", "")
# # #         if api_key != settings.api_key:
# # #             return web.json_response({"success": False, "error": "Unauthorized"}, status=403)
# # #         try:
...
# # #     async def handle_video_feed(request):
# # #         """MJPEG stream for Dashboard live camera widget."""
# # #         response = web.StreamResponse(
# # #             status=200,
# # #             headers={
# # #                 "Content-Type":  "multipart/x-mixed-replace; boundary=frame",
...
# # #     cors.add(app.router.add_post("/pin/update",     handle_update_pin))
# # #     cors.add(app.router.add_post("/camera/enroll",  handle_enroll))
# # #     cors.add(app.router.add_post("/sync-faces",     handle_sync_faces))     # NEW
# # #     cors.add(app.router.add_post("/delete-face",    handle_delete_face))    # NEW
# # #     cors.add(app.router.add_get( "/status",         handle_status))
# # #     cors.add(app.router.add_get( "/video_feed",     handle_video_feed))
...
# # #     tasks = [
# # #         asyncio.create_task(
# # #             uart.listen_loop(uart_rx_queue, pin_from_device_queue), name="uart_rx"
# # #         ),
# # #         asyncio.create_task(event_dispatcher.dispatch_loop(),  name="event_dispatch"),
...
# #   [NEW] TÍCH HỢP TEST ĐỘC LẬP: Thêm hàm auto_trigger_pir() để giả lập kích hoạt
# #         cảm biến PIR mỗi 10 giây (Giúp test trực tiếp trên Web mà không cần cắm phần cứng PIR).
# # """
# # from __future__ import annotations
...
# # # ── GIẢ LẬP PIR CHO MỤC ĐÍCH TEST ───────────────────────────────────────────────

# # async def auto_trigger_pir(detector: PresenceDetector) -> None:
# #     """
# #     HÀM GIẢ LẬP: Tự động đánh thức hệ thống mỗi 10 giây để test chức năng AI/Web
# #     mà không cần nối dây cứng cảm biến PIR.
# #     """
# #     while True:
# #         await asyncio.sleep(10)
# #         from control.presence_detector import State

# #         # Chỉ kích hoạt nếu hệ thống đang ngủ (Tránh việc kích hoạt đè khi đang quét)
# #         if detector.state == State.IDLE:
# #             log_app("info", "👻 [TEST MODE] GIẢ LẬP PIR KÍCH HOẠT! Bắt đầu quét mặt...")
# #             # Tạo task độc lập để tránh block luồng ngủ 10s
# #             asyncio.create_task(detector.on_pir_triggered())
...
# #         # [NEW] Chạy ngầm kịch bản giả lập PIR kích hoạt mỗi 10 giây
# #         asyncio.create_task(auto_trigger_pir(detector),        name="auto_trigger_pir"),
# #     ]
...
#   [FIX] PresenceDetector nhận `broadcast_fn` để đẩy trạng thái face_scan lên Web Dashboard.
#   [FIX] /sync-faces và /delete-face endpoints sử dụng xác thực X-Api-Key.
#   [FIX] Đưa face_sync_pull_loop vào background task để đồng bộ bù khi Pi offline.
#   [FIX] Thêm ký tự \n vào các lệnh UART (Warning Beep, Test Buzzer) để STM32 bắt được.
#   [NEW] TÍCH HỢP TEST ĐỘC LẬP: Thêm hàm auto_trigger_pir() để giả lập kích hoạt PIR mỗi 10s.
# """
# from __future__ import annotations
...
# async def _broadcast_to_backend(event_type: str, data: dict) -> None:
#     """
#     Pi gọi hàm này để đẩy sự kiện lên backend.
#     Đã fix lỗi 422 Schema và chặn spam Base64 làm sập Database.
#     """
#     # 1. CHẶN SPAM FRAME: Web đã có luồng MJPEG Live Stream riêng (port 5000),
#     # TUYỆT ĐỐI KHÔNG đẩy Base64 frame ảnh lên Database Event làm nghẽn mạng!
...
#     try:
#         import json
#         # 2. CHUẨN HÓA DỮ LIỆU: FastAPI cần trường 'message'. Nếu data dùng
#         # chữ 'detail' hay 'reason' thì map lại cho đúng chuẩn để hết lỗi 422.
...
#     async def on_face_fail(result):
#         from config.constants import CMD_WARNING_BEEP
#         # Thêm \n để STM32 nhận lệnh báo tít tít
#         await uart.send(f"{CMD_WARNING_BEEP}\n")
#         await door_ctrl.handle_detection_result(result)
#         try:
#             if result.image_path:
...
#     # Khởi tạo PresenceDetector (BỘ NÃO)
#     detector = PresenceDetector(
#         camera=shared_camera,
#         on_recognized=on_face_ok,
#         on_unknown=on_face_fail,
...
#         uart_send_fn=uart.send,  # <--- [FIX QUAN TRỌNG ĐỂ LCD HIỂN THỊ ĐẾM SỐ LẦN QUÉT]
#     )
...
#         # Chạy ngầm kịch bản giả lập PIR kích hoạt mỗi 10 giây
#         asyncio.create_task(auto_trigger_pir(detector),        name="auto_trigger_pir"),
#     ]
...
async def async_main() -> None:
    ...
    # ── [TÍCH HỢP MỚI] BẮT TAY UART (HANDSHAKE) ───────────────────────────────
    # Khởi tạo Queue hứng dữ liệu UART sớm nhất có thể
    uart_rx_queue:         asyncio.Queue[str] = asyncio.Queue()
    pin_from_device_queue: asyncio.Queue[str] = asyncio.Queue()

    # Bật luồng lắng nghe UART ngầm
    ...
    while time.monotonic() - start_time < 3.0:
        try:
            # Lấy tin nhắn từ Queue với timeout 0.5s để không bị kẹt vĩnh viễn
            msg = await asyncio.wait_for(uart_rx_queue.get(), timeout=0.5)
            if "ACK_PING" in msg:
                handshake_ok = True
                break
            else:
                # Nếu nhận được tin rác khác, quăng lại vào Queue để không làm mất dữ liệu
    ...
    if handshake_ok:
        ...
        # Tắt luồng lắng nghe, đóng cổng UART và thoát chương trình
        listen_task.cancel()
        uart.close()
        await shared_camera.camera_off()
        sys.exit(1)

    # ── Controllers (Chỉ chạy khi Handshake OK) ───────────────────────────────
    door_ctrl  = DoorController(uart_handler=uart)
    alarm_ctrl = AlarmController(uart_handler=uart)
    ...
    # ── Background tasks (Tiếp tục khởi động các task còn lại) ─────────────────
    tasks = [
        listen_task, # Task lắng nghe UART đã bật từ lúc Handshake
        asyncio.create_task(event_dispatcher.dispatch_loop(),  name="event_dispatch"),
        asyncio.create_task(pin_sync_service.run(),            name="pin_sync_w2d"),
        asyncio.create_task(pin_device_svc.run(),              name="pin_sync_d2w"),
        asyncio.create_task(poll_pin_changes(pin_sync_queue),  name="pin_poll"),
