...
NGUYÊN TẮC:
  ...
  - CẢI TIẾN: Áp dụng Ratio Test (So sánh chéo). Nếu ảnh bị nhiễu/thiếu sáng
    khiến distance tăng > 0.5, hệ thống sẽ kiểm tra mức độ "độc tôn".
    Nếu nó vẫn khác biệt rõ ràng với người giống thứ 2, vẫn cho phép mở cửa!
  - [VÁ LỖI BẢO MẬT]: KHÔNG nới lỏng sai số nếu hệ thống chỉ có 1 người.
"""
from __future__ import annotations
...
def compare_faces(
    ...
    try:
        ...
        if best_dist <= margin_tolerance:
            # CHỈ được áp dụng nới lỏng nếu DB có từ 2 người trở lên để dùng Ratio Test.
            # Nếu chỉ có 1 người, ta không có mốc so sánh (second_best_dist), 
            # nới lỏng lúc này sẽ tạo lỗ hổng bảo mật cho người lạ lọt vào.
            if len(candidates) >= 2:
                second_best_dist = candidates[1][1]
                ratio = best_dist / second_best_dist
