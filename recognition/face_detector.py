...
# # pi4/recognition/face_detector.py
# from __future__ import annotations
...
#         frame = await camera.capture_frame()
#         if frame is None:
#             await asyncio.sleep(0.1) # TỐI ƯU: Rút ngắn thời gian chờ để quét nhanh hơn
#             continue

#         # TỐI ƯU: Ảnh đi qua thuật toán bù sáng tự động
#         processed = preprocess(frame)
#         if processed is None:
#             continue
...
#                 # Chỉ lưu ảnh "không có mặt" ở lần thử cuối cùng để tiết kiệm thẻ nhớ
#                 staged = await save_and_stage(processed, prefix=f"no_face_attempt")
#                 best_result.image_path = str(staged) if staged else None
...
#         if name is not None:
#             # NHẬN DIỆN THÀNH CÔNG -> Lưu ảnh success và mở cửa ngay
#             staged = await save_and_stage(processed, prefix=f"success_{name}")
#             best_result.success = True
#             best_result.name = name
#             best_result.image_path = str(staged) if staged else None
...
#             return best_result
#         else:
#             # NHẬN DIỆN THẤT BẠI NHƯNG CÓ MẶT NGƯỜI -> Ghi nhận và thử lại thật nhanh
#             staged = await save_and_stage(processed, prefix=f"unknown_attempt_{attempt}")
#             best_result.image_path = str(staged) if staged else None
#             await asyncio.sleep(0.15)
...
async def detect_with_retry(
    ...
    # Load thông số: Mặc định max_attempts = 5 lần (Lấy từ cài đặt)
    if max_attempts is None:
        max_attempts = settings.max_face_retry
    if tolerance is None:
        tolerance = settings.face_tolerance
    ...
    known = load_known_faces()
    if not known:
        log_app("warning", "Chưa có dữ liệu khuôn mặt, sẽ tự động thất bại")

    best_result = RecognitionResult(success=False, attempts=0, distance=1.0)
    loop = asyncio.get_running_loop()

    # =========================================================================
    # VÒNG LẶP QUÉT ĐÚNG 5 LẦN (NẾU THÀNH CÔNG SẼ DỪNG SỚM)
    # =========================================================================
    for attempt in range(1, max_attempts + 1):
        log_event(EventType.FACE_ATTEMPT, attempt=attempt, max=max_attempts)
        ...
        frame = await camera.capture_frame()
        if frame is None:
            await asyncio.sleep(0.3) # Nghỉ 0.3s (Điểm cân bằng lý tưởng)
            continue

        # Đi qua bộ lọc bù sáng CLAHE
        processed = preprocess(frame)
        if processed is None:
        ...
        if not encodings:
            log_app("debug", f"Không thấy mặt trong khung hình (Lần {attempt})")
            if attempt == max_attempts:
                # Lưu tấm ảnh cuối làm bằng chứng
                staged = await save_and_stage(processed, prefix=f"no_face_attempt")
                best_result.image_path = str(staged) if staged else None
        ...
        # So khớp với Database (Dùng Sai số động)
        name, distance = await loop.run_in_executor(
            None, compare_faces, encodings[0], known, tolerance
        )
        ...
        if name is not None:
            # ✅ QUÉT ĐÚNG NGƯỜI -> Dừng vòng lặp ngay, lưu ảnh, trả về kết quả Mở Cửa
            staged = await save_and_stage(processed, prefix=f"success_{name}")
            best_result.success = True
            best_result.name = name
            best_result.image_path = str(staged) if staged else None
            ...
            # ❌ QUÉT SAI NGƯỜI -> Lưu bằng chứng, nghỉ 0.3s và quét lần tiếp theo
            staged = await save_and_stage(processed, prefix=f"unknown_attempt_{attempt}")
            best_result.image_path = str(staged) if staged else None
            await asyncio.sleep(0.3)

    # =========================================================================
    # KẾT THÚC 5 LẦN MÀ VẪN THẤT BẠI
    # (Sẽ trả kết quả False về để hệ thống bật Nhập PIN và Báo động)
    # =========================================================================
    log_event(
        EventType.FACE_FAILED,
        attempts=max_attempts,
