...
# pi4/communication/uart_protocol.py
"""
Định nghĩa toàn bộ UART protocol giữa Raspberry Pi và STM32.
File này KHÔNG có I/O — chỉ chứa constants và helper functions.
Mọi module khác import từ đây để tránh hardcode string.
...
from __future__ import annotations
from config.constants import (
    CMD_UNLOCK_DOOR, CMD_LOCK_DOOR, CMD_ENABLE_KEYPAD, CMD_STOP_ALARM, CMD_SET_PIN,
    CMD_WARNING_BEEP, CMD_TEST_BUZZER,
    EVT_MOTION_DETECTED,
    RSP_DOOR_OPENED, RSP_DOOR_OPENED_PWD,
    RSP_PASSWORD_FAILED, RSP_ALARM_ACTIVE, RSP_ACK_PIN_SET,
)
...
def frame_lock_door() -> str:
    return build_frame(CMD_LOCK_DOOR)
...
def frame_warning_beep() -> str:
    return build_frame(CMD_WARNING_BEEP)

def frame_test_buzzer() -> str:
    return build_frame(CMD_TEST_BUZZER)
