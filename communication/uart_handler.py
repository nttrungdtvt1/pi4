...
_CMD_NEW_PIN_PREFIX = "CMD_NEW_PIN:"
...
class UartHandler:
    ...
    def connect(self) -> None:
        try:
            self._ser = serial.Serial(
                port=settings.uart_port, baudrate=settings.uart_baud,
                timeout=1.0, write_timeout=2.0,
            )
            self._connected = True
            log_app("info", "UART connected", port=settings.uart_port)
        except serial.SerialException as exc:
            log_event(EventType.UART_ERROR, detail=str(exc), phase="connect")
            raise
    ...
    async def listen_loop(self, event_queue: asyncio.Queue[str], pin_from_device_queue: asyncio.Queue[str] | None = None) -> None:
        log_app("info", "UART listen loop started")
        loop = asyncio.get_running_loop()
        ...
        while True:
            try:
                ...
                # Bắt lệnh đổi PIN từ STM32
                if decoded.startswith(_CMD_NEW_PIN_PREFIX):
                    pin_candidate = decoded[len(_CMD_NEW_PIN_PREFIX):]
                    if _PIN_PATTERN.match(pin_candidate):
                        log_app("info", f"Received new PIN from STM32")
                        if pin_from_device_queue:
                            await pin_from_device_queue.put(pin_candidate)
                    else:
                        log_event(EventType.UART_ERROR, detail=f"Invalid PIN format '{pin_candidate}'")
                    continue
    ...
    async def send(self, frame: str, expect_ack: str | None = None) -> bool:
        async with self._send_lock:
            ...
            for attempt in range(1, UART_SEND_RETRY + 1):
                ...
                try:
                    loop = asyncio.get_running_loop()
                    await loop.run_in_executor(None, lambda: self._ser.write(frame.encode("utf-8")))
                    log_uart("TX", frame.strip())
                    ...
                    try:
                        ...
                        if attempt < UART_SEND_RETRY: await asyncio.sleep(0.1 * attempt)
                except serial.SerialException as exc:
                    log_event(EventType.UART_ERROR, detail=str(exc), phase="write", attempt=attempt)
                    await asyncio.sleep(0.5)
            self._expected_ack = None
