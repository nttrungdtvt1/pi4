...
"""
pi4/services/face_sync_service.py
[VÁ LỖI BẢO MẬT & ĐỒNG BỘ]: Khắc phục triệt để lỗi "Bóng ma" do Race Condition.
Sử dụng Asyncio Lock để đảm bảo mọi thao tác Đọc-Sửa-Ghi trên file pkl và RAM là Độc quyền (Atomic).
"""
from __future__ import annotations
...
from config.settings import settings
from recognition.face_encoder import load_known_faces, save_known_faces
from logging_module.event_logger import log_app, log_event, EventType

# ── Config ────────────────────────────────────────────────────────────────────
POLL_INTERVAL_SECONDS = 60

# ✅ SỬ DỤNG ASYNC LOCK TẬP TRUNG ĐỂ TRÁNH TRANH CHẤP FILE VÀ RAM
...
# ── Core: Thao tác bộ nhớ ĐỘC QUYỀN (Chỉ được gọi trong ngữ cảnh Lock) ───────
def _sync_add_or_update_face(resident_id: int, name: str, encoding_json: str) -> bool:
    """Nạp khuôn mặt mới. Hàm này giả định ĐÃ CÓ LOCK bảo vệ."""
    try:
        try:
            enc_list = json.loads(encoding_json)
            if not isinstance(enc_list, list) or len(enc_list) != 128:
        ...
        # Ghi đè file và CẬP NHẬT TRỰC TIẾP LÊN RAM (Qua hàm save_known_faces)
        if save_known_faces(known):
            log_app("info", f"[face_sync] ✅ Đã nạp và cập nhật RAM khuôn mặt: '{key}'")
            return True
        return False
    except Exception as exc:
        log_app("error", f"[face_sync] Lỗi nạp mặt: {exc}")
        return False

def _sync_remove_face(resident_id: int) -> bool:
    """Xóa khuôn mặt bóng ma. Hàm này giả định ĐÃ CÓ LOCK bảo vệ."""
    try:
        known = load_known_faces(force_reload=True)
        keys_to_remove = [k for k in known if f"__id{resident_id}" in k]

        if not keys_to_remove: 
            return True # Không có để xóa thì coi như thành công

        for key in keys_to_remove:
            del known[key]
            log_app("info", f"[face_sync] 🗑️ Đã chém bóng ma và xóa khỏi RAM: '{key}'")

        # Ghi file mới và ép cập nhật RAM
        return save_known_faces(known)
    except Exception as exc:
        log_app("error", f"[face_sync] Lỗi xóa mặt: {exc}")
        return False
...
async def handle_sync_push(data: dict) -> dict:
    try:
        resident_id = int(data.get("resident_id")) # Ép kiểu an toàn
        name        = data.get("name", "unknown")
        enc_json    = data.get("encoding_json", "")
        ...
        # Lấy Lock độc quyền trước khi ghi bộ nhớ
        async with _sync_lock:
            success = await loop.run_in_executor(None, _sync_add_or_update_face, resident_id, name, enc_json)

        if success:
            asyncio.create_task(_ack_backend(resident_id, True))
            return {"success": True}
        else:
            asyncio.create_task(_ack_backend(resident_id, False, "Failed to apply"))
            return {"success": False}
...
async def handle_delete_push(data: dict) -> dict:
    try:
        ...
        # Lấy Lock độc quyền trước khi xóa bộ nhớ
        async with _sync_lock:
            success = await loop.run_in_executor(None, _sync_remove_face, resident_id)
            ...
            async with aiohttp.ClientSession() as session:
                        ...
                        if pending:
                            ...
                            for item in pending:
                                ...
                                if rid and ejson:
                                    ...
                                    # Xin Lock để ghi
                                    async with _sync_lock:
                                        success = await loop.run_in_executor(None, _sync_add_or_update_face, rid, name, ejson)
                                    asyncio.create_task(_ack_backend(rid, success))
                ...
                async with session.get(url_all, headers=headers, timeout=aiohttp.ClientTimeout(total=10.0)) as resp_all:
                    if resp_all.status == 200:
                        ...
                        # Đọc JSON an toàn
                        res_list = None
                        if "items" in all_data: res_list = all_data["items"]
                        elif "data" in all_data: res_list = all_data["data"]
                        elif "results" in all_data: res_list = all_data["results"]
                        elif isinstance(all_data, list): res_list = all_data

                        if isinstance(res_list, list):
                            # Lấy danh sách ID an toàn trên Web (chuyển về string để dễ đối chiếu mảng)
                            safe_ids = [str(r.get("id")) for r in res_list if "id" in r]

                            # Xin quyền truy cập để đọc RAM an toàn
                            async with _sync_lock:
                                known = load_known_faces(force_reload=False)
                                keys_to_delete = []
                                ...
                                if keys_to_delete:
                                    ...
                                    log_app("warning", f"[face_sync] 🧹 Phát hiện {len(unique_ids_to_delete)} bóng ma! Đang dọn dẹp...")
                                    loop = asyncio.get_running_loop()
                                    for rid_str in unique_ids_to_delete:
                                        # Không cần with_lock ở đây vì ta vẫn đang đứng trong with_lock của khối trên
                                        await loop.run_in_executor(None, _sync_remove_face, int(rid_str))
